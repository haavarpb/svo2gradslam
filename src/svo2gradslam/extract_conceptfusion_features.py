...
def get_dataset(dataconfig_path, basedir, sequence, **kwargs):
    ...
    elif config_dict["dataset_name"].lower() in ["svo"]:
        return SVOIterableDataset(config_dict["svo_file"], **kwargs)
    else:
...
def main():
    ...
    for idx, (color, depth, intrinsics) in tqdm(enumerate(dataset)):
        img: Tensor = color.squeeze()
        masks = mask_generator.generate(img.numpy())
    ...
    for idx, (color, depth, intrinsics) in tqdm(enumerate(dataset)):
        ...
        with torch.amp.autocast("cuda"):
            ...
            _img = preprocess(Image.fromarray(img.numpy(), mode="RGB")).unsqueeze(0)
        ...
        for maskidx in range(len(masks)):
            ...
            img_roi = img[int(_y) : int(_y) + int(_h), int(_x) : int(_x) + int(_w), :]
            img_roi = Image.fromarray(img_roi.numpy(), mode="RGB")
